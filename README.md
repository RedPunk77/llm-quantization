...
Пет-проект про посттренировочное квантование LLM на Apple Silicon
...
INT8 оказался лучшим вариантом по балансу: ускорение 1,43×, экономия памяти 41,5% и 99,81% сохранённой next-token accuracy относительно FP16

INT4 дал максимальную экономию и скорость, но цена заметна по качеству: perplexity выросла на 18%, а сохранение next-token accuracy составило 94,72%

### Условия замеров

- Apple M2, 16 ГБ unified memory
- Qwen2.5-0.5B, одна зафиксированная ревизия исходных весов
- Affine-квантование весов, group size 64, неквантованные параметры в FP16
- Batch size 1, 128 входных и 128 выходных токенов
- 3 прогрева и 10 измерений на каждый формат
- В таблице указаны медианы

Качество измерялось на первых 16 384 токенах test split WikiText-2 с окном 512 и шагом 256

`Quality retention = accuracy_quantized / accuracy_fp16`, где accuracy — доля точных top-1 предсказаний следующего токена

Это конкретная языковая метрика на WikiText-2, а не процент всех способностей модели
...
Учебный квантователь помогает разобрать scale, диапазоны и ошибку восстановления, но сам по себе не является быстрым inference kernel

Для запуска реальной модели используются оптимизированные операции MLX
...
Пиковая память MLX — не то же самое, что полный RSS процесса, а результаты скорости относятся к нашему синхронному greedy-циклу и выбранной длине последовательности

Абсолютную perplexity разных моделей напрямую сравнивать нельзя из-за разных токенизаторов

Accuracy на прикладных задачах вроде MMLU пока не измерялась

## Документация

[Первый запуск](docs/first_run.md) · [Эксперимент Qwen](docs/qwen.md) · [Как объяснить проект на собеседовании](docs/interview.md) · [Описание для резюме](docs/resume.md)
...
Данные: [WikiText-2](https://huggingface.co/datasets/Salesforce/wikitext), лицензии CC BY-SA 3.0 / GFDL

Backend: [MLX-LM](https://github.com/ml-explore/mlx-lm)
